	...
	<view v-show="isShow">
		...
		<view class="pop">
			<view class="flex_col" style="margin-bottom: 20upx;">
				<view class="preview" 
				:style="{'backgroundColor': pickerColor==''?color:pickerColor }"></view>
				<view class="value">
					<input :value="pickerColor==''?color:pickerColor" @input="inputColor" ></input>
				</view>
...
<script>
	export default {
		...
		data() {
			return {
				colorArr:[
					["#fef0ef","#ffedf6","#defbf7","#fdebdd","#fbeacc",
		...
		methods: {
			inputColor(e){
				...
				let nColor = e.detail.value
				if(nColor[0] != '#'){
					nColor = '#' + nColor
				}
				this.pickerColor = nColor
			},
			picker(e) {
